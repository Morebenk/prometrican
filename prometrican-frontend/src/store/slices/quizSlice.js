...
export const startQuiz = createAsyncThunk(
  ...
  async (quizId, { rejectWithValue }) => {
    try {
      const response = await axios.post("/api/v1/attempts", { quiz_id: quizId });
      return response.data;
    } catch (error) {
...
export const submitAnswer = createAsyncThunk(
  "quiz/submitAnswer",
  async ({ attemptId, questionId, selectedChoiceId, timeSpent }, { rejectWithValue }) => {
    try {
...
export const completeQuiz = createAsyncThunk(
  ...
  async (attemptId, { rejectWithValue }) => {
    try {
      const response = await axios.post(`/api/v1/attempts/${attemptId}/complete`);
      return response.data;
    } catch (error) {
